...
### 1. 概要
  - **目的**
    ...
    - インフラ全体の構成を柔軟に設計するため、AWS へ移行しました。
...
### 2. 設計意図
  - **技術選定**
    - 既存の Ruby on Rails アプリがアプリケーション層とデータベース層で構成されているため、AWS への移行においてもレイヤ分離を維持した構成としました。
    - アプリケーション層には、移行および検証フェーズにおける柔軟な制御とトラブルシュートを考慮し、インスタンスレベルで制御可能な ECS（EC2起動タイプ）を採用しました。
  ...
  - **セキュリティ**
    - 認証基盤として Cognito を採用し、OAuth2.0 / OIDC に準拠した安全な認証を採用しました。
  ...
  - **機密情報管理**
    - SSM Parameter Store（ SecureString ）を用いて秘匿情報を管理し、コードへのハードコーディングを排除しました。

  - **拡張性**
    - 将来的に Fargate への移行を見据えた設計としました。
...
### 3. アーキテクチャ設計
  ...
  - **ALB**
    - 外部からのリクエストを受け付け、ECS へルーティング
      - リバースプロキシとして機能し、アプリケーションを直接公開しない構成

  - **ECS（ EC2起動タイプ ）**
    - EC2インスタンス 上で Flaskアプリ をコンテナとして実行
